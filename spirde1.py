...
def weibo_comment():
    ...
    import csv
    import os
    if not os.path.exists('datase.csv'):
        field=open('datase.csv','a',newline='',encoding='utf-8')
        write=csv.DictWriter(field,fieldnames=['评论'])
        write.writeheader()
    else:
        field = open('datase.csv', 'a', newline='', encoding='utf-8')
        write = csv.DictWriter(field, fieldnames=['评论'])
    listuu='https://m.weibo.cn/comments/hotflow?id=4467657128511234&mid=4467657128511234&max_id_type=0'
    listuu=listuu.split('&max_id_type=0')
    for uu in listuu:
        max_id = ""
        max_id_type=0
        uu+="&max_id_type="+str(max_id_type)
        while True:
            try:
                if max_id == "":
                    url = uu
                else:
                    url = uu[:uu.find('max_id')] + 'max_id='+str(max_id) + "&max_id_type="+str(max_id_type)
                print(url)
...
if __name__ == '__main__':
    username = "13244838806"  # 用户名
    password = "Mywork2018"  # 密码
